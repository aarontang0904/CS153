...
import WordleButton from "../component/WordleButton";
...
const App = () => {
  ...
  const [gamesPlayed, setGamesPlayed] =
    useState(0); /* number of games played */
    ...
    if (guess.toLowerCase() == word) {
      ...
      alert(
        "You guessed the word " +
          ...
          " in " +
          (guessNum + 1) +
          (guessNum == 0 ? " guess" : " guesses")
      );
      setGuessNum(guessNum + 1);
      setGameOver(true);
      saveScore(word);
    } else if (!words5.includes(guess.toLowerCase())) {
      /* check that the guess is in the array wards5*/
      guess == "" || alert("Your guess is not a valid word. Please try again.");
    } else if (guessNum == 5 && guess != word) {
      alert(
        "You have already submitted the maximum number of guesses. The word was " +
          word
      );
      setGameOver(true);
      setGuesses(guesses.concat(guess));
    ...
    <SafeAreaView style={styles.container}>
      ...
      {gameOver ? (
        <>
          ...
          />
          <FlatList
            data={Object.keys(scores).map((t) => ({ id: t, val: scores[t] }))}
